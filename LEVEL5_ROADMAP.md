# Level 5 Roadmap — Last Updated: 2026-04-11

> **Project:** Know Your SAP Masters
> **Phase Status:** Phase 9 COMPLETE ✅ | Phase M-Series (Infra) IN PROGRESS 🚧
> **Stack:** FastAPI + Streamlit | Qdrant (Schema+SQL) | Memgraph (Graph RAG) | Redis+Celery
...
| **9** | **Frontend Modernization + Confidence Scoring**             | ✅ **Complete**     | Apr 11   |
| **M** | **Infra/Scale: Memgraph, Qdrant, Redis, Celery**            | 🚧 **IN PROGRESS** | Apr 11   |
...
| 🕸️ Graph Scores Panel             | Node2Vec structural discovery + cross-module bridges      |
...
## Phase M-Series — Infra & Scale (Apr 11, 2026) 🚧

### What was built today

#### Qdrant Migration (Phase M6)
- Unified `VectorStoreManager` to gracefully support both ChromaDB and Qdrant.
- `schema_lookup` and `sql_pattern_lookup` wired to Qdrant.
- Re-seeded all 18 domains into Qdrant (`sap_schema`, `sql_patterns` collections, 384d cosine).
- Migrated Graph Embeddings (`graph_embedding_store.py`) to Qdrant to eliminate ChromaDB v0.4 / v1.5 conflicts.

#### Redis & Celery Scaling (Phase M8)
- Integrated `RedisDialogManager` for distributed conversational state handling.
- App startup establishes connection pools to Redis.
- Added session ID generation (`X-Session-ID`) to the Streamlit UI to correctly bind conversation threads.
- Standalone Celery scaffold established for asynchronous orchestrator execution.

#### Memgraph Integration (Phase M1-M3)
- Substituted NetworkX in-memory Graph RAG with Memgraph via `use_memgraph()` when `MEMGRAPH_URI` is set.
- Cypher-based shortest-path traversal working in backend tests.
