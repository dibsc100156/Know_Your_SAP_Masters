...
**Status:** Phase M1–M6, M8, M10 Complete (Memgraph + Qdrant + Celery + KEDA HPA) ✅ | M7 Pending ⬜
...
| `backend/app/core/graph_embedding_store.py` | Node2Vec + text embeddings in ChromaDB (separate path)                                                                                                                                                                                                   |
...
| `backend/app/core/vector_store.py`          | ChromaDB wrapper — schema + SQL pattern retrieval                                                                                                                                                                                                        |
...
Standard Orchestrator:
  ...
  Step 1:  Schema RAG — schema_lookup() via ChromaDB
  Step 1b:  DDIC Auto-Discover — fires if schema RAG returns empty (Phase 5)
  Step 1.5: Graph Embedding Search — Node2Vec + text hybrid (Pillar 5½)
...
- Separate ChromaDB path: `~/.openclaw/workspace/chroma_graph_db/`
